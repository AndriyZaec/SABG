...
describe("GridRecorder state machine", () => {
  ...
  it("creates exactly two collections and writes only non-empty live frames across two back-to-back matches", async () => {
    const feed = [
      ...
      frame(null), // games empty -> stop, no write
    ...
    // Writes: frame1, frame2, frame3 (first match) + frame5 (second match) = 4 writes.
